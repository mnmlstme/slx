...
describe("complement of selector", function () {
    ...
    it("inverts a conjunction of tag and [attr] selectors", function () {
        expect(slx("a[href]").not().toString())
            .toBe(":not(a),:not([href])");
    });
    ...
    it("inverts a chain of child class selectors", function () {
        expect(slx(".a > .b > .c").not().toString())
            .toBe(":not(.c),:not(.b)>*,:not(.a)>*>*");
    });
    ...
    it("inverts a chain of next class selector", function () {
        expect(slx(".a + .b + .c").not().toString())
            .toBe(":not(.c),:not(.b)+*,:not(.a)+*+*");
    });

    // The following are not valid CSS3, but they will be valid in CSS4
